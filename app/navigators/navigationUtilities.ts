...
export function useNavigationPersistence(persistenceKey: string) {
  const [initialState, setInitialState] = useState<NavigationProps["initialState"]>();
  const isMounted = useIsMounted();
  const initNavState = navigationRestoredDefaultState(Config.persistNavigation);
  const [isRestored, setIsRestored] = useState(initNavState);
  const routeNameRef = useRef<keyof AppStackParamList | undefined>();

  const onStateChange: NavigationProps["onStateChange"] = (state) => {
  ...
  const restoreState = async () => {
    try {
      ...
      const state = savedStateString ? JSON.parse(savedStateString) as NavigationProps["initialState"] : null;
  ...
  return { onStateChange, restoreState, isRestored, initialState };
}
